<template>
  ...
  <v-container class="order-container" style="width: 100%;overflow-x: auto;">
      <!-- success alert -->
      <v-alert v-if="isSucceed" density="comfortable" type="success" variant="tonal"> 
        {{successMessage}}
      </v-alert>
      <!-- error alert -->
              ...
              <v-col cols="12">
                <v-card
                    ...
                    <v-card-text>
                          ...
                          <v-col cols="12" class="title-box">
                             Order # {{order_id ? order_id:'NA'}} 
                          </v-col>
...
export default {
    ...
    methods: {
     imageNotFound(e){
      ...
      async handleEditElement(data){
          this.order_data.selected_products =data.SelectedProducts
      ...
      async updateOrder(){
        ...
        {
          const response = await repository.updateOrder(this.order_data);
            this.clearErrors()
            if(response.data.code == 1){
                  ...
                  this.coupon_order.error_message = this.errors.coupon_code
             }
      ...
      async getSpecificOrder(id){
        const response = await repository.getSpecificOrder(id);
        if(response.data.code ==1){
          this.order_data = response.data.data.order;
          this.original_order_data = JSON.parse(JSON.stringify(response.data.data.order));
          this.order_data.selected_products.forEach(element => {
            this.SelectedProductsIDs.push(parseInt(element.id));
          });
        } else {
          alert('no data order')
        }
      ...
      removeProductFromSelectedProduct(id){
        this.SelectedProductsIDs.splice(this.SelectedProductsIDs.indexOf(parseInt(id)) , 1);
    ...
    watch:{
     ...
     'order_data.shipping_company_id':{
       handler:function(val){
        if(this.edit_shipping_company)
        {
          this.shipping_company_dialog = true
        }
        this.edit_shipping_company = true
      },
      deep:true
     },
    'order_data.shipping':{
    ...
    async created(){
      ...
      await this.getSpecificOrder(this.order_id)
      this.calculateTotal(this.selected_products)

      this.order_data.selected_products.forEach(element => {
          this.selected_products[element.id] = element;
      });
...
<style lang="scss">
  ...
  .action-panel .v-expansion-panel-text__wrapper , 
  .note-panel .v-expansion-panel-text__wrapper
  {
  ...
  .action-panel p{
      ...
      transform: scaleX(1.0704);
  }
  .action-panel .action-row{
    cursor: pointer;
  }
  .action-panel .action-row:hover{
    background-color: rgb(228, 228, 228);
  }
 .v-theme--light .order-head-table th{
 ...
 .order-head-table th{
  ...
  .shipping-comapny-dialog .v-overlay__content{
    width:70% !important;
  }
   .shipping-comapny-dialog .v-theme--light .v-field__field{
    background:white !important;
  }
   .shipping-comapny-dialog  .v-theme--light .v-field--appended{
    background-color:white !important;
  }
  .v-theme--light .shipping-company-card {
    background-color:#f9f9f9 !important;
  }
